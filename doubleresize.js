...
exports.handler = function(event, context) {
    ...
    // Object key may have spaces or unicode non-ASCII characters. RegExp written as a literal value by enclosing the pattern in forward slash characters.
  ...
  var square = _sizesArray[key].square;	
...
if (square == 'yes') {
	...
	console.log(square);
if (square == 'yes')
{var extentw = 150; var extenth = 150;}
else { var extentw = width; var extenth = height;}

                    //this.resize({width: width, height: height, format: 'jpg',})
	 this.resize(width,height).gravity("Center").extent(extentw,extenth).toBuffer(
            ...
            function upload(data, index, next) {
               ...
               // console.log("upload to path : " +  dirName  + "/"  +  _sizesArray[index].destinationPath +  "/" + fileName.slice(0, -4) +   ".jpg");
                // Stream the transformed image to a different folder.
                s3.putObject({
                    Bucket: dstBucket,
                    Key:  dirName +  "/" + _sizesArray[
